...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        temp_path = directory + '/' + str(proposal) + '.json'
        ...
        for comment_id in pdata:
            if comment_id != 0:
                pdata[comment_id]["data"]["total_likes"] = pdata[comment_id]["data"]["total_votes"] - \
                                                           pdata[comment_id]["data"]["total_dislikes"]

        calc = cp.get_graph(pdata)

        cp.show_stats()

        cp.get_profile(pdata)
        ...
        drawProfile(cp.G, cp.profile, 0, cp.titles, pos, justResult=True)
